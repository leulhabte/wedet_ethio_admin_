    ...
    return(
        <Box className={classes.imgContainer}>
            ...
            <Box display="flex" justifyContent="space-between" alignItems="center" className={classes.imgBarComp}>
                <Typography className={classes.imgText}>Upload Post Picture</Typography>
                <IconButton onClick={() => { imgPicker.current.click() }}><CloudUpload className={classes.imgBtn} /></IconButton>
                <input style={{ display: 'none' }} type="file" ref={imgPicker} onChange={_handleImg} accept="image/x-png,image/gif,image/jpeg" multiple="multiple"/>
            </Box>
        </Box>
    )
