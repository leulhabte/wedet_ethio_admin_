...
const InteriorPhoto = ({ data, history, photo, index, ip }) => {
    console.log(ip)
    // Define States
    ...
    const handleRemove = async (id) => {
        try {
            ...
            if (res.status === 200) {
                ...
                setOpen2(false)
                setOpen3(true)
                history.push({ pathname: '/route_garage', state: { photo: photo, data: data, index: index, ip: ip } });
            }
        } catch (e) {
            setOpen2(false);
            setOpen3(true);
    ...
    const fetchPhoto = async (page) => {
        ...
        setError(false);
        try {
            const res = await axios.get(`garage/photos/${data}/?label=INSIDE&p=${page}&per_p=5`,
                {
                    headers: {
                        'Authorization': `Bearer ${Cookies.get('jwt')}`
                    }
