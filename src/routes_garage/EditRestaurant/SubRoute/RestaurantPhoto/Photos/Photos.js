    ...
    const sendPhoto = async(label)=>{
      ...
      try{
        ...
        if (res.status === 200) {
          ...
          setOpen2(false);
          setOpen(true);
          history.push({pathname: '/route_garage', state: {photo: history.location.state.photo, data: history.location.state.data, index: index, ip: history.location.state.ip }});
      }
      }catch(e){
        setOpen2(false);
    ...
    const updateLogo = async()=>{
        try{
          ...
          if(res.status === 200){
            ...
            setOpen2(false);
            setOpen(true);
            history.push({pathname: '/resPhoto_garage'});
          }
        }catch(e){
          setOpen2(false);
          setMessage(e.message)
