...
const Outside = ({ data, history, photo, index, ip }) => {
    ...
    const handleRemove = async (id) => {
        try {
            ...
            if (res.status === 200) {
                ...
                setOpen2(false)
                setOpen3(true)
                history.push({ pathname: '/route_hospital', state: { photo: photo, data: data, index: 2, ip: ip } });
            }
        } catch (e) {
            setOpen2(false);
            setOpen3(true);
    ...
    const fetchPhoto = async (page) => {
        ...
        setError(false);
        try {
            const res = await axios.get(`restaurant/photos/${data}/?label=OUTSIDE&p=${page}&per_p=5`,
                {
                    headers: {
                        'Authorization': `Bearer ${Cookies.get('jwt')}`
                    }
