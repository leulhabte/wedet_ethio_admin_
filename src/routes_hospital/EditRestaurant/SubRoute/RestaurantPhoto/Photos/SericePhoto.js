...
const ServicePhoto = ({ data, history, photo, index, ip }) => {
    ...
    const handleRemove = async (id) => {
        try {
            ...
            if (res.status === 200) {
                ...
                setOpen2(false)
                setOpen3(true)
                history.push({ pathname: '/route_hospital', state: { photo: photo, data: data, index: 3, ip: ip } });
            }
        } catch (e) {
            setOpen2(false);
            setOpen3(true);
    ...
    const fetchPhoto = async (page) => {
        ...
        setError(false);
        try {
            const res = await axios.get(`hospital/photos/${data}/?label=SERVICE&p=${page}&per_p=5`,
                {
                    headers: {
                        'Authorization': `Bearer ${Cookies.get('jwt')}`
                    }
