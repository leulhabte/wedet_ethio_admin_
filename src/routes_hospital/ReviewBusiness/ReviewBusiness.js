...
const ReviewBusiness = ({history}) => {
    ...
    const fetchBusiness = async (page) => {
        try {
            setError(false)
            const res = await axios.get(`hospital?sort=DESC&sort_by=_id&date&p=${page}&per_p=4`,
                {
                    headers: {
    ...
    const fectData = (query)=>{
        let cancel;
        axios({
            url: `hospital/search?text=${query}`,
            method: "GET",
        }).then(res => {
            setUser([])
            setUser(res.data.data)
    ...
    return (
        <Container>
            ...
            {error && <Box display="flex" justifyContent="center" alignItems="center" width="100%" py={30}><Button  className={classes.btn2} onClick={()=>{history.push({pathname: '/gRoute', state: {path: 'resList_hospital', data: null}})}}>Retry</Button></Box>}
        </Container>
    )
}
