...
const EditRestaturant = ({ history }) => {
    ...
    const fetchData = async () => {
        try {
            setLoading(true);
            const res = await axios.get(`/hotel/info/${history.location.state.data}`);
            ...
            res.data.category.map(t =>{
                t === "HOTELS" ? setStateA({ ...stateA, checkboxA1: true }):
                t === "GUEST_HOUSES" ? setStateA({ ...stateA, checkboxA2: true }):
                t === "REAL_ESTATE" ? setStateA({ ...stateA, checkboxA3: true }):
                t === "APARTMENTS" ? setStateA({ ...stateA, checkboxA4: true }):
                t === "RESORTS" ? setStateB({ ...stateB, checkboxB1: true }):
                t === "BARS" ? setStateB({ ...stateB, checkboxB2: true }):
                t === "NIGHTLIFE" ? setStateB({ ...stateB, checkboxB3: true }):
                setStateB({ ...stateB, checkboxB4: true })
            })
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            // Categories
            var _categories = {
                a: stateA.checkboxA1 === true ? 'HOTELS' : "",
                b: stateA.checkboxA2 === true ? 'GUEST_HOUSES' : "",
                c: stateA.checkboxA3 === true ? 'REAL_ESTATE' : "",
                d: stateA.checkboxA4 === true ? 'APARTMENTS' : "",
                e: stateB.checkboxB1 === true ? 'RESORTS' : "",
                f: stateB.checkboxB2 === true ? 'BARS' : "",
                g: stateB.checkboxB3 === true ? 'NIGHTLIFE' : "",
                h: stateB.checkboxB4 === true ? 'ARTS_AND_ENTERTAINMENT' : "",
            }
            ...
            if (res.status === 200) {
                ...
                setOpen2(false);
                setOpen(true);
                history.replace({ pathname: '/resList_hotel' })
            }
                ...
                <Container style={{ background: '#ECF0F3' }}>
                    ...
                    <form onSubmit={handleSubmit}>
                        <NewForm
                            ...
                            handleChangeA={handleChangeA} handleChangeB={handleChangeB}
