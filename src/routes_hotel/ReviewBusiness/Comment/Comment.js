...
const Comments = ({ history }) => {
    ...
    const fetchRating = async () => {
        setLoading(true);
        try {
            const res = await axios.get(`hotel/review/info/${history.location.state.data}`,
                {
                    headers: {
    ...
    const fetchReview = async (page) => {
        setLoading2(true);
        try {
            const res = await axios.get(`hotel/comments/5e5421c453b1f13b544c1197?p=${page}&per_p=3&sort=DESC&sort_by=bestResponse`,
                {
                    headers: {
                        'Authorization': `Bearer ${Cookies.get('jwt')}`
                    }
