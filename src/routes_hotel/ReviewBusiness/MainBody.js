                ...
                <Grid item xs={12}>
                    <Paper className={classes.paper}>
                        <Box width="100%" display="flex" p={3}>
                            <Typography className={classes.tableTitle}>Hotels</Typography>
                        </Box>
                                            ...
                                            return <StyledTableRow key={value.id} hover={true}>
                                                ...
                                                <TableCell align="center">
                                                    <Box display='flex' flexDirection='row' justifyContent="center">
                                                        <IconButton onClick={() => { const user_id = value.id; history.push({ pathname: '/comm_hotel', state: { data: user_id } }) }}><Tooltip title="Review"><Comment classes={{ root: classes.edit }}   style={{color: 'orange'}}/></Tooltip></IconButton>
                                                        <IconButton onClick={() => { const user_id = value.id; history.push({ pathname: '/info_hotel', state: { data: user_id } }) }} ><Tooltip title="Description"><Description classes={{ root: classes.remove }}   style={{color: 'green'}}/></Tooltip></IconButton>
                                                    </Box>
                                                </TableCell>
                                            </StyledTableRow>
                                        })}
